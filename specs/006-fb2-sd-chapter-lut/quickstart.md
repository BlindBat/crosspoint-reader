...
| Partition and monotonic progress (FR-008) | `fb2_book` | the sum of `length` equals the byte span of the fixture's top-level `<section>` elements, measured from the fixture string (not `getBookSize()`, which is that same sum); `calculateProgress` never decreases |
...
| Failed build leaves nothing behind (edge case: low SD space) | `fb2_book` | with stub writes failing mid-assembly, `load()` returns false and no `book.bin`, `chapters.tmp` or `titles.tmp` remains |
...
- a stray flag bit;
- `chapterCount = 0`;
- `chapterCount = 32768`, one above the limit;
- `chapterCount = 32767` in a small file.
...
0. **Baselines, before flashing this branch**: on current firmware, record `[MEM] Free` after opening the worst book at the 256 cap, three first-open timings of the reference anthology, and the EPUB chapter-list window-refresh time (add T033's `[TOC] window` log line to `EpubReaderChapterSelectionActivity` locally for this measurement only; the spec excludes EPUB changes, so it is never committed). Steps 1–3 compare against these.
...
2. **SC-005 chapter list**: in that book, page through the chapter list from top to bottom.
   Time each window refresh from the `[TOC] window` `LOG_DBG` line T033 adds and compare with an EPUB of
