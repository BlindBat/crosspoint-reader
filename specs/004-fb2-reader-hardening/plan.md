...
1. `FB2_MAX_CHAPTERS` 1024 → **256**, one constant, no board gating. The existing cache
   validation already rejects and rebuilds anything above the ceiling, so lowering it needs no
   new code path.
...
**Constraints**: chapter metadata at the ceiling ≤ ¼ of the 148,328 B measured at 1024;
chapter-list memory constant in chapter count; no `sections/<n>.bin` invalidation; no reading
position moved
...
| **II. Memory Is the Design Constraint** | **PASS, and it is the point.** Worst-case chapter metadata drops from ~148 KB to ~36 KB by mechanism: the vector shrinks 4× with the ceiling (1024 → 256 entries × ~36 B on a 32-bit target) and each title's heap block is bounded by a 64-character cap instead of `FB2_CACHE_MAX_STRING`'s 4096. Chapter-list memory becomes `24 × (std::string + ListItem)` instead of `n ×`. Added cost: one `bool` and one `std::string` in the reader, one `uint8_t` per chapter on disk. No bare `new` (the reader already uses `makeUniqueNoThrow`); the window arrays are fixed-size members, not vectors, so no `reserve`/growth question arises. |
...
| **IV. Evidence Over Claims** | **PASS.** Every number cited traces to a file: 148,328 B and 8,944 B from `Fb2BookTest.cpp:340-382`; ~15 KB for the reference book from the same measurement; the per-entry arithmetic is stated as arithmetic, not asserted. The post-change figures are claims until `Fb2BookTest`'s retuned budgets prove them, which is a task, not a footnote. The dithered-cover appearance and the heap figures are explicitly deferred to device verification. |
...
```text
lib/Fb2/
├── Fb2.h                          # FB2_MAX_CHAPTERS 1024 -> 256; FB2_MAX_TITLE_CHARS;
│                                  # SectionInfo.titleDerived
├── Fb2.cpp                        # FB2_CACHE_VERSION 3 -> 4; flags read/write + validation;
│                                  # FB2_CACHE_MIN_SECTION_ENTRY += 1
└── Fb2/
    └── Fb2MetadataParser.{h,cpp}  # first-paragraph label derivation, title cap (L1-L8)
...
|---|---|---|
| `book.bin` v4 bump, which contradicts **SC-006** as written ("No FB2 cache written before this feature is invalidated by it") | The derived-label marker must survive the cache round trip or a book shows different labels on its first and second open. **SC-006 is narrowed to what the reader actually experiences: no page cache (`sections/<n>.bin`) is invalidated and no reading position is lost.** The `book.bin` rebuild is one expat metadata pass per book, on first open after the update — it does not re-paginate anything (verified: `Fb2::load()` falls back to `parseMetadata()` alone, `lib/Fb2/Fb2.cpp:186-206`) | Not bumping means every book already on the device keeps showing `Unnamed` until something unrelated rebuilds its metadata — FR-015 satisfied only for books added after the update. A sidecar flags file means a second file to open, validate and keep in sync to carry one bit per chapter |
...
| `FB2_MAX_TITLE_CHARS`, a second new constant | Without it a stored title is bounded only by 4096, so the ceiling's memory guarantee is 256 × 4 KB in the worst case. It is also what FR-018 needs for derived labels | Capping derived labels only leaves the real-title worst case, which is the one that actually appears in a hostile file |
...
- **One ceiling for every board.** FR-002 permits a board-dependent value; a single constant is
  taken instead. Upgrade path is issue #8's SD-resident LUT, which deletes the constant
