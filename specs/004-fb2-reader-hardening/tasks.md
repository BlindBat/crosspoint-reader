...
**Organization**: one phase per user story. The three stories touch disjoint code paths and can
land in any order, in three separate commits, each individually cherry-pickable.
...
- [ ] T001 Run `bin/run-tests --filter 'fb2'` and record, in the scratchpad, the current pass
      ...
      bytes at the 1024 cap). These are the baseline T004 must be shown to move.

---

## Phase 2: Foundational (Blocking Prerequisites)

**Purpose**: the two files that more than one story edits, changed once so the stories do not
collide in them.

**⚠️ These two tasks block US1 and US3. US2 (the cover page) does not depend on them.**

- [ ] T002 In `lib/Fb2/Fb2.h`, make all three data-model changes in one edit:
      (a) `FB2_MAX_CHAPTERS` `1024` → **`256`**;
      (b) add `static constexpr uint16_t FB2_MAX_LABEL_CHARS = 64;` — the character cap for a
      **derived** label only, never for a real `<title>`;
      (c) add `uint8_t titleDerived = 0;` to `Fb2::SectionInfo` **immediately after `level`**, so
      it packs into the existing padding — verified on riscv32, `sizeof(SectionInfo)` stays 36 B
      (research.md M2). Update the existing `ponytail:` comment on `FB2_MAX_CHAPTERS` to carry
      the measured justification: 99,716 B at 1024 vs 43,732 B at 256, against 222,180 B of
      usable heap; upgrade path is issue #8's SD-resident LUT.
- [ ] T003 In `test/fb2_common/Fb2TestSupport.h`, add two deterministic generators beside
...
**Checkpoint**: `lib/Fb2/Fb2.h` and the fixture header are settled; US1 and US3 can proceed
independently.
...
- [ ] T004 [P] [US1] In `test/fb2_book/Fb2BookTest.cpp`, retune
      `ChapterMetadataAllocatesOneTitlePerChapterAndStaysCapped` to the new ceiling: replace the
      `320 * 1024` capped-book budget with one that fails at 1024 and passes at 256, and record
      the newly measured figures in the test comment the way the existing comment does. **Show it
      failing against `FB2_MAX_CHAPTERS = 1024`** — that demonstration is what makes it a
      regression guard rather than a rubber stamp (Principle V).
- [ ] T005 [P] [US1] In `test/fb2_book/Fb2BookTest.cpp`, add a case that a `book.bin` written at
      ...
      nothing unbounded.
- [ ] T006 [P] [US1] In `test/fb2_book/Fb2BookTest.cpp`, extend
...
- [ ] T010 [US1] In `src/activities/reader/Fb2ReaderChapterSelectionActivity.cpp`'s
      ...
      `rowItems.empty()` guard with `listCount() == 0`.
- [ ] T011 [P] [US1] Mirror the ceiling per FR-007 — the value lives in `lib/Fb2/Fb2.h` and
      nowhere else in code, but three documents quote it: `docs/file-formats.md` (lines ~708 and
      ~725, `1..1024` and "`FB2_MAX_CHAPTERS` (1024)"),
      `specs/003-fb2-nested-chapters/data-model.md:24`, and
      `specs/003-fb2-nested-chapters/research.md:173`. Update each to 256 with a one-line pointer
      to this feature's measurement.
...
- [ ] T017 [US2] Verify V1–V7 in the simulator per [quickstart.md](quickstart.md) §4 steps 1, 2,
      5 and 6, including the deleted-`cover.bmp` case. Record what was observed in the commit
...
- [ ] T018 [P] [US3] In `test/fb2_metadata_parser/Fb2MetadataParserTest.cpp`, cover L1–L8 from
      [contracts/reader-behaviour.md](contracts/reader-behaviour.md) with the T003 generator and
...
- [ ] T020 [US3] In `lib/Fb2/Fb2/Fb2MetadataParser.h/.cpp`, derive the label. Add a context for
...
- [ ] T021 [US3] In `lib/Fb2/Fb2.cpp`, take `book.bin` to v4: bump `FB2_CACHE_VERSION` 3 → 4
...
- [ ] T022 [P] [US3] In `lib/I18n/translations/english.yaml`, add
...
- [ ] T023 [US3] In `src/activities/reader/Fb2ReaderChapterSelectionActivity.cpp`'s
...
- [ ] T024 [P] [US3] In `docs/file-formats.md`, rewrite the `book.bin` section (lines ~700-730)
...
- [ ] T025 [P] Add the `ponytail:` comments the design owes, each naming its ceiling and upgrade
...
- [ ] T026 Run the merge gates in order and fix what they find: `./bin/clang-format-fix -g`,
      `bin/run-tests`, `bin/run-tests --asan`, `pio run -e default`,
      `pio check --fail-on-defect low --fail-on-defect medium --fail-on-defect high`.
- [ ] T027 [P] Build one S3 board — `pio run -e sticky` — since a change can build on C3 and
      fail on S3; CI builds all five environments.
- [ ] T028 Hand the device checks to the human tester per [quickstart.md](quickstart.md) §5 and
      §7: free heap on the reference anthology, the cover page in all four orientations, the
      dithered cover's appearance, and that a position saved before the change reopens on the
      same text with its page cache intact.
...
- **Foundational (T002, T003)**: blocks US1 and US3. **Does not block US2.**
- **US1 (T004–T011)**: after Foundational.
- **US2 (T012–T017)**: independent of everything above — it touches only
  `Fb2ReaderActivity.{h,cpp}` and can start immediately.
- **US3 (T018–T024)**: after Foundational. T023 also depends on T009 (US1's window), the only
  cross-story dependency in the feature.
- **Polish (T025–T028)**: after the stories you intend to ship.

### Within US1

T004–T006 (tests) → T007 (parser) → T008 → T009 → T010 (window, same file, strictly ordered) →
T011 (docs, parallel with any of them).

### Within US3

T018–T019 (tests) → T020 (parser) → T021 (format) → T023 (display, needs T009) ; T022 and T024
are parallel with all of it.

### Parallel Opportunities

- T004, T005, T006 — three independent test cases, same file, one author at a time but no
  ordering between them.
- T011, T022, T024 — documentation and translations, no code dependency.
...
T001 → T002, T003 → T004–T011. Ships the measured memory fix: worst real book's chapter metadata
from 99,716 B (44.9% of usable heap) to 43,732 B (19.7%), and a chapter list whose cost no longer
...
Each story is one commit and one cherry-pick:

1. US1 → `perf: bound FB2 chapter metadata and window the chapter list`
2. US2 → `feat: open FB2 books on their cover`
3. US3 → `feat: label untitled FB2 chapters from their first line`

Nothing in US2 or US3 breaks US1, and each can be dropped without touching the others — except
T023, which needs US1's window to exist.
...
- **The escape hatch**: FR-016 (a derived label must be visually distinguishable) is what forces
  ...
  Dropping FR-016 deletes T019's flag cases, T021, T022, T023's format string and the version
