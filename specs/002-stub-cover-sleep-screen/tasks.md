...
- [X] T013 Make the no-title guard in `renderCoverSleepScreen()` (`src/activities/boot_sleep/SleepActivity.cpp:699`) codepoint-aware instead of byte-aware, so a title of nothing but non-ASCII blanks counts as no title, per FR-005 and spec Edge Cases "Whitespace-only or control-character title" (partial). Today the lambda tests raw bytes: a title of one U+00A0 encodes as `0xC2 0xA0`, the lead byte passes `c > ' '`, and the font *has* that glyph (interval `{0xA0, 0x17F, 0x64}`, `lib/EpdFont/builtinFonts/ubuntu_12_bold.h:3344`), so it renders as a blank advance inside an otherwise empty frame. U+2009, U+200B and U+202F are covered by the font's intervals too and behave the same. Walk the title with the existing `utf8NextCodepoint()` from `lib/Utf8/Utf8.h` and reject `cp <= ' '`, `0x7F`, `0xA0`, `0x2000`-`0x200F` (the font's `{0x200B, 0x2011}` interval makes ZWNJ/ZWJ/LRM/RLM inkless too, while U+2010 hyphen stays ink), `0x202F`, `0x3000` and `0xFEFF`. Reuse that helper — do not add a trimming helper or a new file. U+FEFF alone already renders as the replacement glyph rather than a blank, but is cheapest to include in the same set. Latin, Cyrillic, CJK and combining-mark titles must keep passing.

---

## Phase 10: Convergence

Two findings, one root cause. The no-title guard has now been widened three times (T003 bytes,
T012 `DEL`, T013 the blank set) and still misses inkless codepoints — because a denylist of
codepoints is the wrong shape for the question FR-005 actually asks: *does this title paint any
pixels?* Ask the font instead of guessing, once, and the list stops growing.

- [X] T014 Add `bool textHasInk(int fontId, const char* text, EpdFontFamily::Style style) const` to `lib/GfxRenderer/GfxRenderer.{h,cpp}`: resolve the font with the existing private `resolveTextFontId()` (`lib/GfxRenderer/GfxRenderer.cpp:197-220`, so a CJK title still counts as ink when a fallback family is registered), walk the text with `utf8NextCodepoint()`, and return true on the first codepoint whose `getGlyph()` is non-null **and** has `width > 0 && height > 0`. Zero-size glyphs (space, U+00A0) and missing glyphs are both inkless — that is the whole predicate. Cover it in `test/gfx_renderer/GfxRendererTest.cpp` using `testfonts::tinyFamily()`, whose `' '` is a 0x0 glyph and whose `'A'`-`'F'` are inked, plus `cjkFamily()` for the fallback branch; mutation-verify by relaxing the check to `glyph != nullptr` and showing the space case fails (Constitution V) (FR-005, partial).
- [X] T015 In `renderCoverSleepScreen()` (`src/activities/boot_sleep/SleepActivity.cpp:698-716`), delete the codepoint denylist and the `titleHasInk` loop and call `renderer.textHasInk(UI_12_FONT_ID, bookTitle.c_str(), EpdFontFamily::BOLD)` instead — the same font and style the card draws the title with (FR-005, partial). This closes the codepoints the denylist misses: C1 controls U+0080-U+009F, U+1680, U+2028, U+2029 and U+205F, none of which the UI font covers (`lib/EpdFont/builtinFonts/ubuntu_12_bold.h:3343-3344`, `:3370-3371`). Replace the comment with the correct rationale: a title that paints nothing is no title, and that now includes an all-U+FFFD title, since U+FFFD has no glyph either (last interval ends 0xFEFC, `ubuntu_12_bold.h:3408`) — the claim the current comment makes is false (Constitution IV, contradicts). Re-run the merge gates and re-check the quickstart step 4 fixtures that must keep passing: Latin, Cyrillic, CJK and combining-mark titles.
