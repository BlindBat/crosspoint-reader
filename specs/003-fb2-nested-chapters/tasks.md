...
- [ ] T012 [P] [US1] In `test/fb2_section_cache/Fb2SectionCacheTest.cpp`: a pure-wrapper chapter from `wrapper-only.fb2` builds a section file with `pageCount == 1` (contract C8), and `loadPage(0)` returns a page rather than `nullptr`
- [ ] T013 [P] [US1] In `test/fb2_book/Fb2BookTest.cpp`: `book.bin` v3 round-trip — titles, `fileOffset`, own `length` and `level` survive save/load identically; a v2 file (and a file with a `level` sequence that jumps by more than 1) is rejected and the source is reparsed; `getTocCount() == getSectionCount()` and both index mappings are the identity
...
- [ ] T025 [US2] In `src/activities/reader/Fb2ReaderChapterSelectionActivity.cpp` (`buildRowItems`): prefix each row label with `level * 2` spaces, mirroring the EPUB list's `(level - 1) * 2` indent at `src/activities/reader/EpubReaderChapterSelectionActivity.cpp:65` (EPUB TOC levels are 1-based, FB2 levels are 0-based). Keep building `rowLabels` once in `onEnter()` and keep the existing `tr(STR_UNNAMED)` substitution; add no new user-facing string and no new list component
...
- **Phase 6 (Polish)**: needs every shipped story
