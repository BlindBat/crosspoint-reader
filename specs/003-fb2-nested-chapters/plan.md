...
| **II. Memory Is the Design Constraint** | PASS with a measured trade. Deleting `tocEntries` removes one `std::string` copy of every chapter title, so flat books use less RAM after this change. Nested books hold more chapters (reference: 4 → 66 ≈ +5 KB), bounded by `FB2_MAX_CHAPTERS = 1024` and a per-title cap. The parser stack is a `std::vector` with `reserve()` before any `push_back`. No bare `new`; new buffers use `makeUniqueNoThrow`. No new allocation in a render-loop path. |
