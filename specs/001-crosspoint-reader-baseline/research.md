...
- Tests: no ASan job in CI; two suites on system expat; no simulator environment in-repo; `--quick` omits two suites; bash-only tooling.

## 5. Test program summary

43 suites (see `plan.md` Technical Context): alloc_guards, book_metadata_cache, chapter_html_slim_parser, chapter_xpath_resolver, combining_marks, credential_integrity, css_parser, dict_zip, dictionary, differential_rounding, direct_pixel_writer, epub_section, fb2_book, fb2_cover_extractor, fb2_filename, fb2_metadata_parser, fb2_section_cache, fb2_section_parser, font_cache_manager, font_decompressor, html_to_plain_text, hyphenation_eval, image_dims_probe, jpeg_to_bmp, ligature_guard, minibidi_arabic, opds_filename, opds_parser, ota_asset_selection, page_link, persistable_stores, png_decode, progress_mapper, release_json_parser, sdcard_font, streaming_json_parser, text_block_layout, token_boundary, utf8_compose, web_dav_paths, xml_parser_utils, xtc_parser, zip_file. Instruments: `AllocCounter` (operator new shim), `AllocGuard` (malloc interposer with simulated OOM), an 87-file malformed-input corpus with universal-invariant and pinned-outcome tests, hyphenation accuracy thresholds against pyphen ground truth.
