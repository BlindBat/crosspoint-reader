...
- **FR-192** *(fork-only)*: Host-reachable logic MUST be covered by the host test program (43 suites) runnable plain and under ASan/UBSan via `bin/run-tests`, with deterministic fixtures, a malformed-input corpus for parsers, and pinned "documents current limitation" tests for known defects.
