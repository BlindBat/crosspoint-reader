...
**Purpose**: Remaining work found on 2026-09-20 by assessing the code against spec.md and the constitution through the ten issues on the fork's tracker (`BlindBat/crosspoint-reader`), filed during hardware verification of 003-fb2-nested-chapters. Only the issues that contradict a requirement this spec sets are carried here: #4, #5 and #10 ask for behaviour no FR requires (#10 explicitly satisfies FR-009), and #6, #7 and #8 match the decisions recorded in `specs/003-fb2-nested-chapters/research.md:173`, `plan.md:162` and `docs/file-formats.md:708`, so re-deciding the chapter cap is a spec amendment, not convergence work. Ordered HIGH → MEDIUM; no constitution violation found among them.

### HIGH

- [X] T191 Stop reader screenshots failing outright for every book whose title is 64 or more UTF-8 bytes, which is most Cyrillic, Greek and CJK titles (fork issue #1): all four readers copy the title with `snprintf(info.title, sizeof(info.title), "%s", ...)` into the 64-byte `ScreenshotInfo::title` (`src/activities/reader/EpubReaderActivity.cpp:2606`, `src/activities/reader/Fb2ReaderActivity.cpp:474`, `src/activities/reader/XtcReaderActivity.cpp:330`, `src/activities/reader/TxtReaderActivity.cpp:342`) and `FsHelpers::sanitizePathComponentForFat32` (`lib/FsHelpers/FsHelpers.cpp:200-216`) then copies byte-by-byte to `maxLen - 1` with no UTF-8 boundary check, so a multi-byte character can be cut in half — `Марсианские хроники. Полное издание` is 66 bytes and byte 62 is `0xD0`, a lead byte whose continuation falls outside the buffer. SdFat's `mbToCp` rejects that dangling sequence (`(str + n) > end` returns `nullptr`, `SdFat/src/common/FsUtf.cpp:79`), so the `Storage.mkdir` in `ScreenshotUtil::saveFramebufferAsBmp` (`src/util/ScreenshotUtil.cpp:113-116`) fails and the save returns false with "Failed to save screenshot". Fix it once in the sanitizer rather than at the four copy sites — it is the single funnel every path component passes through, it cannot recover the lost continuation byte but dropping the trailing incomplete sequence yields a valid name, and `buildFilename` already performs exactly this walk-back on its separate `strlen(buf) > 255` branch (`src/util/ScreenshotUtil.cpp:50-56`); extend the doc comment at `lib/FsHelpers/FsHelpers.h:54-58` to state the guarantee. Per Constitution V, pin it with a test in the existing `test/platform_helpers/ScreenshotUtilTest.cpp` (whose `reader()` helper at `:41-45` reproduces the production truncation exactly) asserting that a 66-byte Cyrillic title yields a path that decodes as valid UTF-8, shown failing against the unfixed sanitizer; `LongestPossibleTitleStaysInsideTheFat32PathBudget` at `:122-129` misses this because it uses ASCII. Per FR-187 and US15/AC2 (contradicts)
- [X] T192 Make `CMD:SCREENSHOT` deliver the whole framebuffer instead of the first few KB (fork issue #2): `src/main.cpp:632` writes all 48,000 bytes in one `logSerial.write(buf, bufferSize)` and discards the return value, while `logSerial.setTxTimeoutMs(1)` at `:363` — deliberate, so logging never stalls the main loop with no host draining the port — makes `HWCDC::write` abandon everything the USB CDC endpoint cannot accept within 1 ms; measured, a continuously-reading host receives `SCREENSHOT_START:48000`, 5,713 bytes and no `SCREENSHOT_END`, so the spec's framing is never closed. Keep the load-bearing 1 ms and loop on the remainder instead, using the count each `write` returns, with `platform::yield()` between attempts and an overall deadline from `platform::millis()` so a host that stops reading cannot wedge the loop; emit `SCREENSHOT_END` on both the complete and the timed-out path, and `LOG_ERR` the short count. Per Constitution III and V the drain loop must be host-reachable: put it in `lib/Platform/` over the existing `platform::ByteSink` (`lib/Platform/PlatformSeam.h:33-40`), have `src/main.cpp` pass a thin `logSerial` adapter, and cover it in `test/platform_helpers` with a sink that accepts a fixed number of bytes per call plus one that accepts none, asserting full delivery and bounded give-up respectively. Per FR-186 and US16/AC2 (contradicts)

### MEDIUM

- [X] T193 Make the FB2 reader menu's Take screenshot row do something (fork issue #3): `Fb2ReaderActivity::openReaderMenu` (`src/activities/reader/Fb2ReaderActivity.cpp:103-120`) opens the shared `EpubReaderMenuActivity`, and `ReaderMenu::buildMenuItems` always emits the Take screenshot row (`src/activities/reader/EpubReaderMenuItems.h:61`), but `Fb2ReaderActivity::onReaderMenuConfirm` (`:122-211`) implements only SELECT_CHAPTER, TEXT_SETTINGS, GO_TO_PERCENT, GO_HOME and DELETE_CACHE and drops every other action into `default:` at `:208-210`, so choosing the row returns to the page and writes nothing. The plumbing already exists on both sides — `Fb2ReaderActivity::getScreenshotInfo()` is implemented at `:470-479` and the Power+Down combo works — so mirror the EPUB pattern: a `pendingScreenshot` member alongside `pendingPercentJump` (`src/activities/reader/Fb2ReaderActivity.h:23`), set under a `RenderLock` followed by `requestUpdate()` as at `src/activities/reader/EpubReaderActivity.cpp:908-915`, and consumed with `ScreenshotUtil::takeScreenshot(renderer)` after the page is painted in `renderBook()` (`Fb2ReaderActivity.cpp:328-418`) as at `EpubReaderActivity.cpp:1425-1428`. Note T191 gates the user-visible outcome: until the sanitizer is fixed this row still fails for long non-Latin titles however it is triggered. The same `default:` also silently swallows Night Mode, Frontlight, Look Up, Auto Turn, Show page as QR, Toggle Bookmark and Sync Progress, which FR-105 does not require of the FB2 reader but which the shared menu still displays — decide in this task whether to gate those rows in `buildMenuItems` behind a capability argument or leave them, and say which in the commit message. Per US15/AC2 and FR-105 (partial)
