...
// POSIX/stdio-backed HalStorage stub for the EPUB Section suite.
//
// Same shape as test/fb2_common/stubs/HalStorage.h with one deliberate
// difference: openFileForWrite opens "wb+" (read+write) instead of "wb",
// because Section::loadPageDuringBuild reads already-written pages back
// through the same handle the build is appending to (the device handle is
// O_RDWR). Section always seeks between read and write phases, which is
// exactly what stdio update streams require.
...
class HalStorage {
 public:
  ...
  // "wb+": the Section build handle must support read-back (see file comment).
