...
std::string KOReaderDocumentId::calculate(const std::string& filePath) {
  ...
  // Buffer for reading chunks. On the heap: CHUNK_SIZE alone is four times the
  // 256-byte stack budget, and this runs on the sync path next to TLS buffers.
  auto buffer = makeUniqueNoThrowForOverwrite<uint8_t[]>(CHUNK_SIZE);
  if (!buffer) {
    LOG_ERR("KODoc", "OOM: %u bytes of hash buffer", static_cast<unsigned>(CHUNK_SIZE));
    return "";
  }
