...
bool Fb2CoverExtractor::extractBinaryToJpeg(const std::string& tempJpegPath) const {
  ...
  do {
    ...
    const int len = fb2File.read(buf, 1024);
...
bool Fb2CoverExtractor::extract() const {
  ...
  if (!extractBinaryToJpeg(tempJpegPath)) {
    LOG_ERR("FB2", "Failed to extract cover binary");
    return false;
  }
  ...
  if (!success) {
    LOG_ERR("FB2", "Failed to convert cover JPEG to BMP");
    Storage.remove(outputBmpPath.c_str());
  } else {
    LOG_DBG("FB2", "Generated cover BMP");
  }
  return success;
}
...
bool Fb2CoverExtractor::extractThumb(const std::string& thumbPath, int height) const {
  ...
  if (!extractBinaryToJpeg(tempJpegPath)) {
    LOG_ERR("FB2", "Failed to extract cover binary for thumbnail");
    return false;
  }
  ...
  if (!success) {
    LOG_ERR("FB2", "Failed to generate thumbnail BMP");
    Storage.remove(thumbPath.c_str());
  } else {
    LOG_DBG("FB2", "Generated thumbnail BMP");
  }
  return success;
}
