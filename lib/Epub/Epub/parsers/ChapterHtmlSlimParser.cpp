...
void ChapterHtmlSlimParser::flushPendingAnchor() {
  ...
  if (std::find(tocAnchors.begin(), tocAnchors.end(), pendingAnchorId) != tocAnchors.end()) {
    if (currentPage && !currentPage->elements.empty()) {
      ...
      if (!currentPage) {
        LOG_ERR("EHP", "Failed to create page at a chapter boundary");
        return;
      }
      currentPageNextY = 0;
...
void ChapterHtmlSlimParser::startNewTextBlock(const BlockStyle& blockStyle) {
  ...
  if (!currentTextBlock) {
    LOG_ERR("EHP", "Failed to create text block");
    return;
  }
...
void ChapterHtmlSlimParser::addLineToPage(std::shared_ptr<TextBlock> line, const uint32_t visibleOffset) {
  ...
  if (!currentPage) {
    currentPage.reset(new (std::nothrow) Page());
    if (!currentPage) {
      LOG_ERR("EHP", "Failed to create page for a line");
      return;
    }
    currentPageNextY = 0;
    currentPageVisibleOffsetSet = false;
  ...
  if (currentPageNextY + lineHeight > viewportHeight) {
    ...
    currentPage.reset(new (std::nothrow) Page());
    if (!currentPage) {
      LOG_ERR("EHP", "Failed to create page after a line break");
      return;
    }
    currentPageNextY = 0;
    currentPageVisibleOffsetSet = false;
...
void ChapterHtmlSlimParser::makePages() {
  ...
  if (!currentPage) {
    currentPage.reset(new (std::nothrow) Page());
    if (!currentPage) {
      LOG_ERR("EHP", "Failed to create page for a paragraph");
      return;
    }
    currentPageNextY = 0;
    currentPageVisibleOffsetSet = false;
