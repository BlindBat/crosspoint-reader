...
void wrapDefinitionText(const char* text, const uint32_t n, const int maxWidth, const int spaceWidth,
                        const SpanMeasurer& measurer, std::vector<WrappedLine>& lines) {
  lines.clear();
  lines.reserve(n / 32 + 8);

  const auto measureSpan = [&measurer](const char* span, const size_t len) {
    return measurer.measure(measurer.ctx, span, len);
  };
  ...
  while (i < n) {
    ...
    const int tokenWidth = measureSpan(text + tokenStart, tokenLen);
    ...
    if (lineEnd == lineStart) {
      ...
      lineWidth = tokenWidth;
               ...
               tokenStart + tokenLen - lineStart <= UINT16_MAX) {  // span len must fit WrappedLine::len
      lineEnd = tokenStart + tokenLen;
