...
inline SettingInfo buildFontSizeSetting(const SdCardFontRegistry* registry) {
  // Captured by copy: the lambdas outlive this call (the entry is stored in the
  // settings list), so they must not reference the registry.
...
// Every entry the firmware knows about, before any board pruning. Built from
// scratch; getSettingsList() below owns the single cached instance.
inline std::vector<SettingInfo> buildBaseSettingsList() {
...
//
// The list is built once and handed out by reference, so repeated calls (every
// settings save and load walks it, and so does each web settings request) cost
// nothing. Board capabilities are re-read on every call and the list is rebuilt
// if they ever change, so the pruning below stays per-call correct.
//
// Two constraints come with the shared instance. A rebuild replaces the vector,
// which invalidates any reference or iterator a caller is still holding, so it
// must not happen while one is walking the list; in practice it never does,
// because BoardConfig::selectDevice() runs in gpio.begin() before the first call
// (main.cpp) and nothing changes the capabilities afterwards. And the cache is
// written without a lock, so callers must stay on one task — today every one of
// them (settings load/save, the settings screen, the web settings handlers) runs
// on the main task, never on the render task.
inline const std::vector<SettingInfo>& getSettingsList() {
  static std::vector<SettingInfo> cached;
  static bool cachedValid = false;
  static bool cachedTouch = false;
  static bool cachedHomeKey = false;

  const bool touch = BoardConfig::hasTouch();
  const bool homeKey = BoardConfig::hasHomeKey();
  if (cachedValid && touch == cachedTouch && homeKey == cachedHomeKey) {
    return cached;
  }

  std::vector<SettingInfo> v = buildBaseSettingsList();
  if (!touch) {
  ...
  }
  {
    ...
    if (it != v.end()) {
      *it = buildFontSizeSetting(nullptr);
    }
  }
...
// The same list with the SD font registry and the discovered dictionaries folded
// in: the font-family entry becomes registry-aware when SD fonts are installed,
// the font-size options come from the active family, and a dictionary picker is
// appended to the Reader category. By value, because those inputs change while
// the device runs (fonts uploaded over the web server, dictionaries copied to
// the card) and each caller consumes its result within one call.
inline std::vector<SettingInfo> getSettingsList(const SdCardFontRegistry* registry,
                                                const std::vector<DictionaryEntry>* dictionaries = nullptr) {
  std::vector<SettingInfo> v = getSettingsList();
  ...
  {
    // The active family's installed point sizes, which the cached list cannot
    // know: it is built without a registry.
    auto it = std::find_if(v.begin(), v.end(), [](const SettingInfo& s) { return s.nameId == StrId::STR_FONT_SIZE; });
    if (it != v.end()) {
