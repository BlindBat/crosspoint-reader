...
void CrossPointWebServerActivity::startAccessPoint() {
  ...
  stopDnsServer();
  dnsServer = new (std::nothrow) DNSServer();  // raw: stopDnsServer() owns the delete
  if (dnsServer == nullptr) {
    LOG_ERR("WEBACT", "OOM: DNS server; the captive portal will not redirect");
  } else {
