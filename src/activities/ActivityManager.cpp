...
void ActivityManager::replaceActivity(std::unique_ptr<Activity>&& newActivity) {
  // A null pointer means the allocation failed (makeUniqueNoThrow): stay where
  // we are rather than tearing down the screen we are on.
  if (!newActivity) {
    LOG_ERR("ACT", "OOM: activity not created; staying on the current screen");
    return;
  }
...
void ActivityManager::pushActivity(std::unique_ptr<Activity>&& activity) {
  if (!activity) {
    LOG_ERR("ACT", "OOM: activity not created; nothing pushed");
    return;
  }
  if (pendingActivity) {
    // Should never happen in practice
