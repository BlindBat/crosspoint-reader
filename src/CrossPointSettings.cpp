...
bool CrossPointSettings::saveToFile() const {
  ...
  serialization::writePod(outputFile, inlineImages);
  serialization::writePod(outputFile, coverSleepScreen);
...
bool CrossPointSettings::loadFromFile() {
  ...
  do {
    ...
    if (++settingsRead >= fileSettingsCount) break;
    serialization::readPod(inputFile, inlineImages);
    if (++settingsRead >= fileSettingsCount) break;
    serialization::readPod(inputFile, coverSleepScreen);
    if (++settingsRead >= fileSettingsCount) break;
    // New fields added at end for backward compatibility
  } while (false);
